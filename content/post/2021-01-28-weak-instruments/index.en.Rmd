...
```{r plot, echo=FALSE}
ggplot(df, aes(x=n,y=beta,color=Version)) + geom_line() + geom_hline(yintercept = 0.3) + xlab('Sample size') + ylab('Effect estimate') +  scale_y_continuous(limits=c(-0.5,1))
```

(). Now we'll consider a case where $z$ is a weaker instrument. We'll mostly use the same assumptions as above, but now suppose $\text{corr}(x_1,z)=0.1$:
...
\end{align}
$$
Taking expectations again (and noting that the last term drops out sinze $Z$ and $U$ are uncorrelated):
$$
\begin{align}
\mathbb{E}(\hat{\beta}) &= \beta + \mathbb{E}[(\hat{\gamma} Z' \hat{\gamma} Z)^{-1}\hat{\gamma} Z'X_2\alpha] \\ 
&=\beta + \frac{ \alpha}{\hat{\gamma}} \mathbb{E}[(Z'Z)^{-1}Z'X_2 ] \\
&= \beta + \frac{ \alpha \delta}{\hat{\gamma}}
\end{align}
$$
where I let $\delta=\mathbb{E}[(Z'Z)^{-1}Z'X_2 ]$ be the expected ().